...
public class loadData {
        ...
        return args -> {
            ...
            if (hotelRepository.count() == 0) {
                ...
                hotelRepository.findAll().forEach(hotel -> {
                    if (roomTypeRepository.findByHotelId(hotel.getId()).isEmpty()) {
                        seedRoomTypes(roomTypeRepository, random, roomNames, hotel);
                    }
                });
            }
    ...
    private void seedRoomTypes(RoomTypeRepository roomTypeRepository, Random random, String[] roomNames, Hotel hotel) {
        for (int j = 0; j < 3; j++) {
